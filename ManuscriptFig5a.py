...
from rouzo import Fig5a as Fig5a
...
#x_obs, y_obs = tools.readfile("../reef/examples/TopoObs_Corinth_Prof1.dat")
x_obs, y_obs = tools.readfile("../reef/examples/TopoObs_Corinth_Prof0.dat")
...
# c1 = 170.    #Erosion rate Profile 3
# d1 = 4.4      #Initial slope Profile 3
# e1 = 6.      #Wave base Profile 3
# f1 = 0.45    #Uplift Rate Profile 3
...
n1 = 420    # Number of matrix elements: (ipmax-ipmin) / ipstep, round up to full number
...
fig.savefig('Figs/Fig5a/Stats-Loglikelihood.pdf')

fig=plt.figure()
plt.plot(chain.stats["prop_S"][1:])
fig.savefig('Figs/Fig5a/Stats-prop_S.pdf')

fig=plt.figure()
plt.plot(chain.stats["accept_ratio"][1:])
fig.savefig('Figs/Fig5a/Stats-accept_ratio.pdf')

fig=plt.figure()
plt.plot(chain.stats["parameter_accept_ratio"][1:])
fig.savefig('Figs/Fig5a/Stats-parameter_accept_ratio.pdf')
...
fig, fig2 = Fig5a.profile(x_n, y_n, x_obs_n, y_obs_n, best)

all_loglikes = chain.stats["loglikelihood"][stp:, :]
best_loglike = all_loglikes[best, :]
np.savetxt("Figs/Fig5a/BestLogLike.txt", best_loglike)

# Sea-level plot
xsl = np.arange(0, tstart, 1)
ysl = chain.posterior_predictive["e"][0, :, :][stp:]
fig, fig2 = Fig5a.sealevel(xsl, ysl, best)

mean = np.mean(ysl, axis=0)
median = np.percentile(ysl[:, :], 50, axis=0)
best_sl = ysl[best, :]
np.savetxt("Figs/Fig5a/MeanSL.txt", mean)
np.savetxt("Figs/Fig5a/MedianSL.txt", median)
np.savetxt("Figs/Fig5a/BestSL.txt", best_sl)
...
fig.savefig("Figs/Fig5a/Histogram-2D-ER-IS.pdf")
...
fig.savefig("Figs/Fig5a/Histogram-2D-ER-WB.pdf")
...
fig.savefig("Figs/Fig5a/Histogram-2D-IS-WB.pdf")
...
fig.savefig("Figs/Fig5a/Histogram-2D-ER-UR.pdf")

# Box 2.5
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 77][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 79][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("Figs/Fig5a/Histogram-2D-IS-UR.pdf")
...
fig.savefig("Figs/Fig5a/Histogram-2D-WB-UR.pdf")
