...
def Plot_Prof0(i, xz, prems, der, ow, dz, dt, dt_profile, tpt, ti, tmax, x, z, colors):
...
def disparam(slcurve, dt, dx, u, upmax, G, V0, slopi, tmax, tmaxb):
# Displays simulation parameters

    print('******************************************************************')
    print('Vertical rate = ', u, 'mm/y - Total displacement = ',upmax )
...
# Adds nodes at the beginning of the profile in case it is too short for the simulation
    ...
    return np.argmax(x>xold[prems])-1 ,np.argmax(x>xold[der])-1, x, y
