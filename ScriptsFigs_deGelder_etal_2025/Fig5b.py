...
def profile(x_n, y_n, x_obs, y_obs, best):
    ...
    ax.legend(loc=4)
    plt.savefig("Figs/FigS4b/Profiles.pdf")
    ...
    plt.savefig("Figs/FigS4b/Profile_median_percentiles.pdf")

    best_prof = x_n[best, :]
    np.savetxt("Figs/FigS4b/MeanProfile.txt", mean)
    np.savetxt("Figs/FigS4b/MedianProfile.txt", p50)
...
def sealevel(xsl, ysl, best):
    ...
    cbar.set_label("Probability", fontsize=fs - 4)
    ax.legend(loc=4)
    plt.savefig("Figs/FigS4b/Sea-Level.pdf")
    ...
    plt.savefig("Figs/FigS4b/Sea-Level_median_percentiles.pdf")
