...
prop_S = np.array([1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05,
                   ...
                   1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05,
                   1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1, 1, 25, 25, 25, 0.2,
                   0.2, 0.2, 0.5, 0.5, 0.5, 0.05, 0.05, 0.05])
...
c2 = 400.    #Erosion rate Profile 2
c3 = 250.    #Erosion rate Profile 3
d1 = 11.    #Initial slope Profile 1
d2 = 5.      #Initial slope Profile 2
d3 = 4.5      #Initial slope Profile 3
e1 = 8.5    #Wave base Profile 1
e2 = 8.5      #Wave base Profile 2
e3 = 8.5      #Wave base Profile 3
f1 = 1.26  #Uplift Rate Profile 1
f2 = 0.75    #Uplift Rate Profile 2
f3 = 0.45    #Uplift Rate Profile 3
...
n1 = [420, 151, 101]    # Number of matrix elements: (ipmax-ipmin) / ipstep, round up to full number
...
def param(x):
    ...
    return tnew, enew, u0, slopi, v0, zb
...
def loglike(x):
    ...
    for i in range(n_topo):
        x, y, dz, xmin, xmax, ymin, ymax, x_ini, y_ini, dt_final = \
            main.reef(t, e, tmax, u0[i], slopi[i], v0[i], zb[i], dx_reef, dt0, xmin, xmax, ymin, ymax)
...
fig.savefig('../Figs/Fig5d/Stats-Loglikelihood.pdf')

fig=plt.figure()
plt.plot(chain.stats["prop_S"][1:])
fig.savefig('../Figs/Fig5d/Stats-prop_S.pdf')

fig=plt.figure()
plt.plot(chain.stats["accept_ratio"][1:])
fig.savefig('../Figs/Fig5d/Stats-accept_ratio.pdf')

fig=plt.figure()
plt.plot(chain.stats["parameter_accept_ratio"][1:])
fig.savefig('../Figs/Fig5d/Stats-parameter_accept_ratio.pdf')
...
for i in range(n_topo):
    ...
    fig, fig2 = Fig5d.profile(x_n, y_n, x_obs, y_obs, best, i)

all_loglikes = chain.stats["loglikelihood"][stp:, :]
best_loglike = all_loglikes[best, :]
np.savetxt("../Figs/Fig5d/BestLogLike.txt", best_loglike)

# Sea-level plot
xsl = np.arange(0, tstart, 1)
ysl = chain.posterior_predictive["e"][0, :, :][stp:]
fig, fig2 = Fig5d.sealevel(xsl, ysl, best)

mean = np.mean(ysl, axis=0)
median = np.percentile(ysl[:, :], 50, axis=0)
best_sl = ysl[best, :]
np.savetxt("../Figs/Fig5d/MeanSL.txt", mean)
np.savetxt("../Figs/Fig5d/MedianSL.txt", median)
np.savetxt("../Figs/Fig5d/BestSL.txt", best_sl)
...
fig.savefig('../Figs/Fig5d/Histogram-2D-36.pdf')

# Box 1.37
df = pd.DataFrame({"Age (ka)" : chain.samples[:, 72][stp:],
                    "SL Elevation (m)" : chain.samples[:, 73][stp:]})
fig = sns.jointplot(data=df, x="Age (ka)", y="SL Elevation (m)", kind="hex", palette="colorblind")
plt.savefig("../Figs/Fig5d/Histogram-2D-37.pdf")

# Box 1.38
df = pd.DataFrame({"Age (ka)" : chain.samples[:, 74][stp:],
                    "SL Elevation (m)" : chain.samples[:, 75][stp:]})
fig = sns.jointplot(data=df, x="Age (ka)", y="SL Elevation (m)", kind="hex", palette="colorblind")
plt.savefig("../Figs/Fig5d/Histogram-2D-38.pdf")

# Box 2.1
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 76][stp:],
                    "Initial slope (%)" : chain.samples[:, 79][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Initial slope (%)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-IS-P1.pdf")

# Box 2.2
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 76][stp:],
                    "Wave base depth (m)" : chain.samples[:, 82][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-WB-P1.pdf")

# Box 2.3
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 79][stp:],
                    "Wave base depth (m)" : chain.samples[:, 82][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-WB-P1.pdf")

# Box 2.4
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 76][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 85][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-UR-P1.pdf")

# Box 2.5
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 79][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 85][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-UR-P1.pdf")

# Box 2.6
df = pd.DataFrame({"Wave base depth (m)" : chain.samples[:, 82][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 85][stp:]})
fig = sns.jointplot(data=df, x="Wave base depth (m)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-WB-UR-P1.pdf")

# Box 2.7
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 77][stp:],
                    "Initial slope (%)" : chain.samples[:, 80][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Initial slope (%)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-IS-P2.pdf")

# Box 2.8
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 77][stp:],
                    "Wave base depth (m)" : chain.samples[:, 83][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-WB-P2.pdf")

# Box 2.9
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 80][stp:],
                    "Wave base depth (m)" : chain.samples[:, 83][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-WB-P2.pdf")

# Box 2.10
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 77][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 86][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-UR-P2.pdf")

# Box 2.11
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 80][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 86][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-UR-P2.pdf")

# Box 2.12
df = pd.DataFrame({"Wave base depth (m)" : chain.samples[:, 83][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 86][stp:]})
fig = sns.jointplot(data=df, x="Wave base depth (m)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-WB-UR-P2.pdf")

# Box 2.13
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 78][stp:],
                    "Initial slope (%)" : chain.samples[:, 81][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Initial slope (%)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-IS-P3.pdf")

# Box 2.14
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 78][stp:],
                    "Wave base depth (m)" : chain.samples[:, 84][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-WB-P3.pdf")

# Box 2.15
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 81][stp:],
                    "Wave base depth (m)" : chain.samples[:, 84][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Wave base depth (m)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-WB-P3.pdf")

# Box 2.16
df = pd.DataFrame({"Erosion rate (mm/yr)" : chain.samples[:, 78][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 87][stp:]})
fig = sns.jointplot(data=df, x="Erosion rate (mm/yr)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-ER-UR-P3.pdf")

# Box 2.17
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 81][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 87][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-IS-UR-P3.pdf")

# Box 2.18
df = pd.DataFrame({"Wave base depth (m)" : chain.samples[:, 84][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 87][stp:]})
fig = sns.jointplot(data=df, x="Wave base depth (m)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("../Figs/Fig5d/Histogram-2D-WB-UR-P3.pdf")
