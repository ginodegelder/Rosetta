...
from ScriptsFigs_deGelder_etal_2025 import FigS4b as FigS4b
...
ipstep = 0.5  # related to uncertainty in terrace height
...
n1 = 387    # Number of matrix elements: (ipmax-ipmin) / ipstep, round up to full number
...
plt.savefig("Figs/FigS4b/Stats-Loglikelihood.pdf", format="pdf", bbox_inches="tight")

fig=plt.figure()
plt.plot(chain.stats["prop_S"][1:])
plt.savefig("Figs/FigS4b/Stats-prop_S.pdf", format="pdf", bbox_inches="tight")

fig=plt.figure()
plt.plot(chain.stats["accept_ratio"][1:])
plt.savefig("Figs/FigS4b/Stats-accept_ratio.pdf", format="pdf", bbox_inches="tight")

fig=plt.figure()
plt.plot(chain.stats["parameter_accept_ratio"][1:])
plt.savefig("Figs/FigS4b/Stats-parameter_accept_ratio.pdf", format="pdf", bbox_inches="tight")
...
fig, fig2 = FigS4b.profile(x_n, y_n, x_obs_n, y_obs_n, best)

mean = np.mean(x_n, axis=0)
median = np.percentile(x_n[:, :], 50, axis=0)
best_prof = x_n[best, :]
np.savetxt("Figs/FigS4b/MeanProfile.txt", mean)
np.savetxt("Figs/FigS4b/MedianProfile.txt", median)
np.savetxt("Figs/FigS4b/BestProfile.txt", best_prof)

all_loglikes = chain.stats["loglikelihood"][stp:, :]
best_loglike = all_loglikes[best, :]
np.savetxt("Figs/FigS4b/BestLogLike.txt", best_loglike)

# Sea-level plot
xsl = np.arange(0, tstart, 1)
ysl = chain.posterior_predictive["e"][0, :, :][stp:]
fig, fig2 = FigS4b.sealevel(xsl, ysl, best)

mean = np.mean(ysl, axis=0)
median = np.percentile(ysl[:, :], 50, axis=0)
best_sl = ysl[best, :]
np.savetxt("Figs/FigS4b/MeanSL.txt", mean)
np.savetxt("Figs/FigS4b/MedianSL.txt", median)
np.savetxt("Figs/FigS4b/BestSL.txt", best_sl)
...
fig.savefig("Figs/FigS4b/Histogram-2D-ER-IS.pdf", format="pdf", bbox_inches="tight")
...
fig.savefig("Figs/FigS4b/Histogram-2D-ER-WB.pdf", format="pdf", bbox_inches="tight")
...
fig.savefig("Figs/FigS4b/Histogram-2D-IS-WB.pdf", format="pdf", bbox_inches="tight")
...
fig.savefig("Figs/FigS4b/Histogram-2D-ER-UR.pdf", format="pdf", bbox_inches="tight")

# Box 2.5
df = pd.DataFrame({"Initial slope (%)" : chain.samples[:, 77][stp:],
                    "Uplift Rate (mm/yr)" : chain.samples[:, 79][stp:]})
fig = sns.jointplot(data=df, x="Initial slope (%)", y="Uplift Rate (mm/yr)", kind="hex", palette="colorblind")
fig.savefig("Figs/FigS4b/Histogram-2D-IS-UR.pdf", format="pdf", bbox_inches="tight")
...
fig.savefig("Figs/FigS4b/Histogram-2D-WB-UR.pdf", format="pdf", bbox_inches="tight")
